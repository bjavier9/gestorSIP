...
export class AuthService {
  ...
  public async register(requestingUser: AuthPayload, input: RegisterUserInput): Promise<RegisterUserResult> {
    ...
    if (!targetRole || !ASSIGNABLE_ROLES.has(targetRole)) {
      throw new ApiError('VALIDATION_INVALID_ROLE', 'El rol solicitado no es v�lido para nuevos usuarios.', 400);
    }
    ...
    if (requesterRole === UserRole.SUPERADMIN) {
      ...
      if (!requestingUser.companiaCorretajeId) {
        throw new ApiError('FORBIDDEN', 'Tu token no contiene una compa��a asociada.', 403);
      }
      if (targetCompaniaId && targetCompaniaId !== requestingUser.companiaCorretajeId) {
        throw new ApiError('FORBIDDEN', 'No puedes registrar usuarios en otra compa��a.', 403);
      }
      targetCompaniaId = requestingUser.companiaCorretajeId;
    }

    if (!targetCompaniaId) {
      throw new ApiError('VALIDATION_MISSING_FIELD', 'No se pudo determinar la compa��a destino.', 400);
    }
    ...
    if (!ente.companiaCorretajeId) {
      throw new ApiError('ENTE_INVALID', 'El ente no tiene una compa��a asociada.', 400);
    }

    if (ente.companiaCorretajeId !== targetCompaniaId) {
      throw new ApiError('FORBIDDEN', 'El ente pertenece a una compa��a distinta a la solicitada.', 403);
    }
    ...
    try {
      ...
      throw new ApiError('USUARIO_COMPANIA_CREATION_FAILED', error?.message || 'No se pudo crear la asociaci�n usuario-compa��a.', 500, error);
    }
  }
